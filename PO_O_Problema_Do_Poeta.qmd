...
1.  A biblioteca de otimização que iremos usar é o *Pyomo*. Usamos um "alias" para importar todos os módulos da biblioteca, assim podemos acessar as funções e métodos diretamente chamando `pyo`. Por exemplo, chamados `pyo.ConcreteModel()` para criar um modelo em branco.
...
### Declarar índices, parâmetros e variáveis de decisão
...
1.  Acessando o objeto `modelo`, criamos um novo item dentro dele chamado `restricao_area_pinus`, e para construir a restrição buscamos a função `Constraint` dentro da biblitoeca do `pyomo`.
2.  Dentro da função, é onde escrevemos usando python a restrição que foi montanda usando a linguagem matemática no tópico de [restrições](#restrições): $x_1 ≤ 40$ . Aqui, a variável $x_1$ é representada por `modelo.x1_area_pinus`, e os 40 hectares de limite, são representados diretamente pelo valor do dicionário `area_max['pinus']`

#### Área máxima para a floresta nativa

```{python}
modelo.restricao_area_nativas = pyo.Constraint(       # <1>
...
#### Tempo limite de manejo para cada floresta
